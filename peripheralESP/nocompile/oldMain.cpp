...
bool rf_updateTokens()
{
  ...
  // Wait for buffer to fill with a 500ms timeout limit
  unsigned long waitStart = millis();
  while (!Serial2.available()) 
  {
    if (millis() - waitStart > 500) {
      Serial.println("[ERROR] RasPi UART Timeout");
      rfDataTokens[0] = "";
      return false; // Break out so loop() isn't permanently blocked
    }
  }
...
float rf_getRawAvgReading()
{
  ...
  // Guardrail: If a timeout occurred, the first token will be empty. 
  // Return 0.0 (or your baseline noise floor) to prevent processing empty data.
  if (rfDataTokens[0] == "") {
      return INVALID_RF_READING; 
  }

  float readingSum = 0;

  // Use the constant RF_TOKEN_COUNT instead of the pointer length
  for (int i = 1; i < (RF_TOKEN_COUNT - 1); i++)
  {
    readingSum += rfDataTokens[i].toFloat();
  }

  return readingSum / (RF_TOKEN_COUNT - 1);
}
...
bool calibrate_rf()
{
  ...
  while (end_time - start_time <= 2000)
  {
    float rawReading = rf_getRawAvgReading();
    if (rawReading == INVALID_RF_READING) return false;
...
void setup()
{
  ...
  pir.periodLen_ms = normalPollPeriod_ms; 
  photo.periodLen_ms = normalPollPeriod_ms;
  rf.periodLen_ms = normalPollPeriod_ms;
  ...
  {
    String rfCalibrateMsg = "ERROR: HackRF is NOT booted up.~";
    sendLogMsgToCtrlr(rfCalibrateMsg);
    rf.enabled = false;
  }
...
void loop()
{
  ...
  */
  ...
  // Update the clock once every 60 seconds (60000 ms)
  ...
  if (gotReading)
  {
    ...
    Serial.println("Sending poll reading");
